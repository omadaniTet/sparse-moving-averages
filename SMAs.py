...
class DYAL:
    ...
    def whether_to_use_queue(self, c, ema_prob, q, qprob, qcount):
        if ema_prob <= 0:
            return True
        is_higher = self.qprob_is_sufficiently_higher(
    ...
    # (by a coin head-prob equal to ema prob), return True (ema_prob
    # is not a good model and should be revised/reset).
    def qprob_is_sufficiently_higher(
    ...
    # Is the 'observed' prob, as judged/estimated by the queue prob,
    # substantially lower than the modeled (EMA) prob?  If the queue
    # prob (proportion observed) is so small that, with very high
    # confidence, the observation count could not have been generated
    # by the ema_prob (by a coin with prob equal to ema prob), then
    # return True (implying that the ema_prob is not a good model and
    # should be reset).
